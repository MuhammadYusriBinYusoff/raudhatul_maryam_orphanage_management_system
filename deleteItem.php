...
<body>
    ...
    <!-- Team Start -->
     ...
     $tql="SELECT * FROM requestItem WHERE itemId ='$ID'"; 
    $results = mysqli_query($conn, $tql);  //ni untuk cari dalam database
    $rows=mysqli_fetch_assoc($results); //return a set of rows number arrays available in the table //untuk tahu row berapa
    $itemName=$rows["itemName"]; //just nak assign to value to user_name based on row yang dah dapat tahu tadi
    $itemQuantity=$rows["itemQuantity"];
    $itemUnits=$rows["unit"];
    $category=$rows["category"];
    $dates=$rows["dates"];

    $rql = "INSERT INTO history(itemName, itemQuantity, unit, category, dates) VALUES ('$itemName','$itemQuantity', '$itemUnits','$category','$dates')" ;
    ...
    while($row=mysqli_fetch_array($querys)){
     ...
     $sql = "DELETE FROM requestitem WHERE itemId = '$ID'" ;
